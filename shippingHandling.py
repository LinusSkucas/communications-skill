...
def start_advertisement_loop(name):
	...
	# BUG: ***Does Not Work With WINDOWS***
	# Linux fix to find active network interface name.
	except:
		interfaceName = subprocess.check_output("ip addr show | awk '/inet.*brd/{print $NF}'", shell=True).decode('utf-8').rstrip()
		ip = netifaces.ifaddresses(interfaceName).get(netifaces.AF_INET)[0].get("addr")
	info = ServiceInfo(
		"_http._tcp.local.",
